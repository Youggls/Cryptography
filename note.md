...
- 教科书式的 Rabin(**考试可能换成RSA**) 加密体制满足 IND-CCA 吗？为什么？
  ...
  - $Gen$：输入 $\lambda$，执行 $GenModulus(\lambda)$ 获得 $(N,p,q)$，满足 $p=q=3\ \text{mod}\ 4$，输出公钥 $pk=N$ 和私钥 $sk=(p,q)$
  ...
  - 构造 $c'$，询问解密预言机，可以得到 $m'$ 满足 $c'=m'^2\ \text{mod}\ N$
  - 构造 $c''=c'c$，询问解密预言机，可以得到 $m''$ 满足 $c''=m''^2\ \text{mod}\ N$
  - 那么 $m''=mm'$，可得 $m$
